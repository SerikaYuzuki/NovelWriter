...
- `NovelAI`のpure domainや個人用AIの動作はAI公開を意味しない。Experimental UIの前提はexact SDK／CLI／Node version・path・hash、lockfile／package integrity、request専用empty cwd／`CODEX_HOME`、environment allowlist、Keychain、OS-level file隔離、本文／prompt／response／key／pathの非ログ、direct child回収と同一group停止、group脱出拒否、parent death後の回収、event／process limit、local artifact inventory。B4-Aのapproval契約とB4-Bの非実行exact Node inspectorは実装済みだが、production catalogは空である。B3 packager result／self manifest、B4-Bのpath／SHA／Mach-O／requested architecture別CDHash／署名observation、observed version／local probeを承認情報へ昇格させず、universal Mach-Oもapproval扱いせず、same-user post-verify swap／Node version／actual process／complete inventory／immutable binding／OS隔離を解決済みとせず、anti-rollbackを実装済みと扱わない。失敗後のpartial destinationは自動cleanup済みと扱わず手動で隔離・削除する。same-process supervisorの合成成功だけでgrandchild reap／orphan-freeを主張しない。arm64／x86_64、bundled runtime、nested signing／公証は公開Release Gateへ延期する。公開SDKにないtool無効化／upstream token cap／ephemeral optionをあるように扱わず、Experimental UIで未保証と表示する
