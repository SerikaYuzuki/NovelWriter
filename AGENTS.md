...
- Phase 0(基盤)/ Phase 1(最小執筆環境)/ Phase 2(Editorプラグイン基盤 + 自動インデント)/ Phase 3(基本操作強化)/ Phase 4-1(メタデータ基盤 + 章メモ + 文字数)/ Phase 4-2(キャラクター管理・最小)/ Phase 4-3(キャラクター ⇄ 本文の連携)/ Phase 4-4(プロット / シーンカード・最小)/ Phase 4-5(伏線・フラグ管理)完了
- 動くもの: 章リスト(追加・選択・タイトル編集・削除・並べ替え)、NSTextView エディタ、自動字下げ(改行で全角スペース、`「` で字下げ解除)、章メモ、文字数表示、検索ジャンプ、スナップショット保存、`.novelpkg` 自動保存(2秒デバウンス)、Cmd+Q 時の終了前保存、起動時の前回作品読み込み
- 次: Phase 4-6(資料添付)。**サブフェーズ分解済みの作業指示書 = [docs/PHASE4.md](docs/PHASE4.md)。1サブフェーズ = 1PR で進めること**
