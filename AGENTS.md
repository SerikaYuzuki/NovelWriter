...
**設計の正は [docs/DESIGN.md](docs/DESIGN.md)、決定の記録は [docs/DECISIONS.md](docs/DECISIONS.md)(D-001〜)。この2つを読んでから作業すること。** 次にやるべきタスクは DESIGN.md の「11. 直近の次タスク」にある(**現在進行中の作業指示書は [docs/PHASE5.md](docs/PHASE5.md)**。Phase UI2 と Phase 4 の完了記録は [docs/UIDESIGN.md](docs/UIDESIGN.md) / [docs/PHASE4.md](docs/PHASE4.md))。
...
- 動くもの: 章リスト(追加・選択・タイトル編集・削除・並べ替え)、NSTextView エディタ、自動字下げ(改行で全角スペース、`「` で字下げ解除)、章メモ、文字数表示、キャラクター管理、登場章ジャンプ、プロットカード、伏線管理、資料添付、検索ジャンプ、スナップショット保存・一覧・確認付き復元、`.novelpkg` 自動保存(2秒デバウンス)、Cmd+Q 時の終了前保存、起動時の前回作品読み込み、作品の新規・開く・別名保存
- 次: **Toolbar-2(一段ツールバー + カスタマイズ)**。詳細は [docs/TOOLBAR.md](docs/TOOLBAR.md)。並行して Phase 5-1 も着手可([docs/PHASE5.md](docs/PHASE5.md))
