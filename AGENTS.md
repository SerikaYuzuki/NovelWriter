...
- AIの現在地: `NovelAI`のpure domain、EditorKitのAI selection transaction、providerへ送らないApp-level local context、送信前／適用前stale検査、決定論的fake、共通のpreview／確認／cancel／diff／Copy／Apply UIを実装。これらは別bundle・別保存rootの`FUMINIWAExperimental` targetだけに入り、通常版はbuild graph監査で`NovelAI`とAI UIを除外する。Codex sidecar v1のNode／Swift mock、canonical deployment manifest v1、exact SDK 0.147.0の合成CLI capture、合成helper用Darwin native process supervisor、arm64向け固定21-file allowlist packager、Experimental native manifest verifier、B4-Aのcompile-time approval契約、B4-Bの非実行exact Node native inspector、B4-Cのprobe-only suspended actual-process identity inspectorに加え、B4-DのExperimental-only／mock-onlyなabstract interactive transport sequencing契約を実装した。B4-Dはcontent-free factoryからfreshな1-request channelを開き、sealed payload budget由来のabsolute request deadlineをrun開始時に固定し、`hello`だけを書いてexact request／runtimeの単独`ready`をframe boundaryで照合した後にだけexact sealed `start`を書く。`started` → 単一terminal → EOF、cancel／timeoutのfirst-wins、terminal後最大1秒のEOF drain、global live-channel reuse拒否、cleanup error優先／raw error redactionを合成testで固定した。ただし具象production channel／factory／callsiteは0件で、process／Node／SDK／CLI／network／key／実原稿も使わない。production catalogは空、B4-C childは一度もresumeされず、B4-D transportへ変換されていない。これはtransport sequencing feasibilityだけで、実runtime B4-Dの完了／GOではない
- 今後「商業化」として扱う範囲は、実装・機能・UI/UX・データ安全・性能・アクセシビリティ・互換性・ビルド／配布技術に限定する(D-042)。価格、法務、販促、決済、事業運用は、ユーザーから明示依頼がない限り調査・提案・ロードマップ化しない
- 次: 個人用AIは **Codex SDK sidecar隔離feasibilityのB4-E** として、closed execution closure／native broker／helperとapproval／actual identity／OS-level read／exec隔離を結合し、完全なloaded artifact inventoryとimmutable verify-to-useを固定する。B4-Dのabstract mock transportを具象process transportとして扱わず、B4-C observationもB3 candidate／self manifest／B4-B observation／local probeと同じく承認値へ自動昇格させない。B4全体でanti-rollbackを実装済みと扱わず、same-uid外部`SIGCONT`やmapped-vnode／in-place mutationも未解決のままである。全Gate後にだけ既存共通UIへのadapter接続を再評価する。公開Releaseの次は引き続き **Package Validator Gate**、続いて **External Change / Conflict Gate** とする(D-040 / D-043 / D-046〜D-053)
...
- `NovelAI`のpure domainや個人用AIの動作はAI公開を意味しない。Experimental UIの前提はexact SDK／CLI／Node version・path・hash、lockfile／package integrity、request専用empty cwd／`CODEX_HOME`、environment allowlist、Keychain、OS-level file隔離、本文／prompt／response／key／pathの非ログ、direct child回収と同一group停止、group脱出拒否、parent death後の回収、event／process limit、local artifact inventory。B4-Aのapproval契約、B4-Bの非実行exact Node inspector、B4-Cのprobe-only suspended identity inspector、B4-Dのabstract mock interactive sequencing契約は実装済みだが、production catalogは空で、具象production channel／factory／callsiteは0件である。B3 packager result／self manifest、B4-Bのpath／SHA／Mach-O／requested architecture別CDHash／署名observation、B4-C observation、observed version／local probeを承認情報へ昇格させず、universal Mach-Oもapproval扱いせず、same-user外部resume／mapped-vnode／in-place mutation／Node version／B4-B SHAとactual processのauthority binding／complete inventory／immutable binding／OS隔離を解決済みとせず、anti-rollbackを実装済みと扱わない。失敗後のpartial destinationは自動cleanup済みと扱わず手動で隔離・削除する。B4-Cの協調的なconstructor／`main` marker 0だけでadversarialなuser-code非実行を主張せず、B4-Dの合成transportを実runtime B4-Dの完了／GOへ一般化せず、same-process supervisorの合成成功だけでgrandchild reap／orphan-freeを主張しない。arm64／x86_64、bundled runtime、nested signing／公証は公開Release Gateへ延期する。公開SDKにないtool無効化／upstream token cap／ephemeral optionをあるように扱わず、Experimental UIで未保証と表示する
