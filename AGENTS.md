...
- AIの現在地: `NovelAI`のpure domain、EditorKitのAI selection transaction、providerへ送らないApp-level local context、送信前／適用前stale検査、決定論的fake、共通のpreview／確認／cancel／diff／Copy／Apply UIを実装。これらは別bundle・別保存rootの`FUMINIWAExperimental` targetだけに入り、通常版はbuild graph監査で`NovelAI`とAI UIを除外する。Codex sidecar v1のNode／Swift mock、canonical deployment manifest v1のNode primitive、exact SDK 0.147.0を合成CLIだけで駆動するcapture、合成helper用Darwin native process supervisorまで実装した。実CLI／network、native manifest verifier、Keychain、OS-level隔離、parent death後の回収は未実装
- 今後「商業化」として扱う範囲は、実装・機能・UI/UX・データ安全・性能・アクセシビリティ・互換性・ビルド／配布技術に限定する(D-042)。価格、法務、販促、決済、事業運用は、ユーザーから明示依頼がない限り調査・提案・ロードマップ化しない
- 次: 個人用AIは **Codex SDK sidecar隔離feasibility** として、allowlist packager／native manifest verifier、exact Node runtime、監査済みlauncher／parent-death境界を固定する。実装済みsupervisorは同一groupへのTERM／KILL、direct child reap、post-reap group空観測までであり、grandchild reap、group脱出、appのSIGKILL／crash／power loss後の回収を保証しない。専用cwd／`CODEX_HOME`、environment allowlist、file-read拒否、memory／CPU／process limit、Keychain credential pipe、artifact inventoryを実証してから既存共通UIへadapterを接続する。その後、同じUIへ独立したOpenRouter API adapterを追加する。公開Releaseの次は引き続き **Package Validator Gate**、続いて **External Change / Conflict Gate** とする(D-040 / D-043 / D-046 / D-047 / D-048)
...
- `NovelAI`のpure domainや個人用AIの動作はAI公開を意味しない。Experimental UIの前提はexact SDK／CLI／Node version・path・hash、lockfile／package integrity、request専用empty cwd／`CODEX_HOME`、environment allowlist、Keychain、OS-level file隔離、本文／prompt／response／key／pathの非ログ、direct child回収と同一group停止、group脱出拒否、parent death後の回収、event／process limit、local artifact inventory。same-process supervisorの合成成功だけでgrandchild reap／orphan-freeを主張しない。arm64／x86_64、bundled runtime、nested signing／公証は公開Release Gateへ延期する。公開SDKにないtool無効化／upstream token cap／ephemeral optionをあるように扱わず、Experimental UIで未保証と表示する
