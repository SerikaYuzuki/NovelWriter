...
**Phase 2(Editor基盤強化)まで完了**。章の作成・並べ替え・本文編集・`.novelpkg` への自動保存に加え、Editorプラグイン基盤と日本語小説向け自動字下げ(改行で全角スペース、`「`/`『` で字下げ解除、IME変換中は不介入)が動く。次は Phase 3(章タイトル編集・章削除・検索ジャンプなど。[docs/DESIGN.md 11章](docs/DESIGN.md) を参照)。
