...
個人用AIのExperimental基盤は、Codex sidecar protocol、exact SDK 0.147.0の合成capture、Darwin process supervisor、B3 deployment packager／verifier、B4-A empty approval contract、B4-B非実行exact Node inspectorに加え、B4-Cのprobe-only suspended actual-process identityまで合成検証した。B4-Cは固定argv／empty environment／`/private/var/empty`／null stdio／新PGIDでchildを`START_SUSPENDED`起動し、actual PIDのprocess identityとdynamic SecCode／no-network exact caller-supplied CDHash／非ad-hocを照合する。成功時もresumeせず`SIGKILL` + direct `waitpid`で回収し、architecture／CDHashだけの非authority値を返す。production catalogは意図的に空で、B4-C observationを承認や実行capabilityへ昇格させない。timeoutはbest-effort lifecycle境界でasync hard return上限ではなく、same-uid外部`SIGCONT`、mapped-vnode／in-place mutation、Node version、B4-B SHA／approvalとの結合、complete loaded inventory／immutable binding、OS隔離は未達である。次のB4-D interactive `hello`／`ready`／`start`、B4-E closed execution closureと残Gateが完了するまで、`codex_sdk` runtime、実SDK／CLI、key、network、原稿送信はNO-GOである。通常版のtarget構成は変更していない(D-046〜D-052)。
