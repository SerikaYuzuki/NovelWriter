...
**Phase 1(最小執筆環境)**。`NovelKit`(NovelCore / NovelStorage / EditorKit)に加え、macOS アプリ本体 `NovelApp` を実装中。章の作成・並べ替え・本文編集・`.novelpkg` への自動保存が一通り動く([docs/DESIGN.md 8章 開発ロードマップ](docs/DESIGN.md)を参照)。
