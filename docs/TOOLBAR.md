...
**状態: Toolbar-1 / Toolbar-2 / UI-FIX-4 / UI-FIX-5 / UI-REF-1〜6 / UI-POL-4 完了。** 上部 chrome、Toolbar起点のpopover、セクション別の追加操作は実装済み。次は Phase 5-1へ進む。
...
- 既定ではEditor左端に「話を追加」、中央に話メモ・スナップショット・プロットカード追加、右端に話内検索を置く。Outline固有の追加操作はOutline上の固定項目として表示する
...
`ToolbarItem` の ID はリリースをまたいで不変にする。作品名、章ID、配列位置などの動的な値を ID に使わない。UI-POL-4で既定配置を変更したため、toolbar 全体の ID は `novelwriter.workbench.v3` へ版上げした。
