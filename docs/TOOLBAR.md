...
- 既定ではEditor左端に「話を追加」、中央に話メモ・スナップショット・書き出し、右上のprimary actionにプロットカード参照、右端に話内検索を置く。プロットカード参照は右側から開くスライド式ペインで、選択中の章のカードを表示する。Outline固有の追加操作はOutline上の固定項目として表示する
