...
- 既定ではEditor左端に「話を追加」、中央にクラウド同期・話メモ・スナップショット・書き出し、右上の独立した`secondaryAction`にプロットカード参照、右端に話内検索を置く。プロットカード参照は右側から開くスライド式ペインで、選択中の章のカードを表示する。Outline固有の追加操作はOutline上の固定項目として表示する
...
| `workbench.plot.card.rail` | プロットカード | 執筆時のみ表示 | 右上固定 | `secondaryAction` のボタンで選択中の章のカードを右側のスライド式ペインに表示 |
...
`ToolbarItem` の ID はリリースをまたいで不変にする。作品名、章ID、配列位置などの動的な値を ID に使わない。プロットカードをmacOSの`secondaryAction`へ移し、クラウド同期を編集操作列へ追加したため、toolbar 全体の ID は `novelwriter.workbench.v6` へ版上げした。既存のv5カスタマイズは新しい既定配置へ移行する。
