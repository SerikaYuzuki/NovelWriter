...
- **内容**:
  ...
  - `formatVersion` を "2" に上げる。読み込みは "1" / "2" を受理し、"1" は新ファイルの欠損を「空」として読む(マイグレーション = 次回保存で "2" になる)。"3" 以上は従来どおりエラー
...
- **内容**:
  ...
  2. `PlotCard.chapterID` と `Flag` の張った章／回収章は章単位の参照として維持する。今回 `episodeID` 参照は追加しない。現行の章メモは移行時に生成される話のメモへ移し、章自体のメモは追加しない。
  3. `.novelpkg` は v3 とし、manifest が章順と章内の話順を持つ。本文とメモは `EpisodeID` ベースのファイルへ分離する。v1 / v2 は各旧章の `ChapterID` と章タイトルを保ち、その本文・メモをタイトル「本文」の1話へ移して読み込む。読み込みは v1 / v2 / v3、保存は v3 とする。
  ...
  5. 現在の Phase 5 出力仕様は `Chapter` が本文を持つ前提なので、本移行と関連 UI Fix を Phase 5-1 より先に完了する。
