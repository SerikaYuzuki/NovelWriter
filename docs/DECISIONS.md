...
- **内容**:
  ...
  9. Apple版MVPでは利用者がworkを削除するUI／APIを出さない。誤ったidentity推定による別work削除を避け、削除・CloudKit tombstone・複数端末retentionは別Decisionで設計する。
