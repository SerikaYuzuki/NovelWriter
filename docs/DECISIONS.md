...
- **実装状況 (2026-08-09)**: 別app target／scheme／bundle ID／既定保存root、通常版とのbuild graph分離、App-level local context、fake provider、provider-neutralな共有AI UI、Codex sidecar v1、manifest v1、exact SDKの合成CLI capture、Darwin native process supervisor、B3 packager／native verifier、D-050のB4-A compile-time approval契約、D-051のB4-B非実行exact Node inspectorまで実装した。production catalogは空で、実Codex／OpenRouter adapter、実CLI／network、approved runtime、Node version検査、suspended actual-process identity、interactive transport、complete loaded inventory／immutable verify-to-use binding、Keychain、OS-level隔離、parent death後のprocess回収は未実装であり、本決定のExperimental Gateを完了した意味ではない。
