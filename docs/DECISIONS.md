...
- **内容**:
  ...
  3. D-033のR5を、IME確定後の `　「` / `　『` だけでなく、対応する括弧ペア `　「」` / `　『』` にも適用する。キャレットが括弧内またはペア直後にある場合、行頭の全角スペースだけを正規の置換経路で削除し、括弧ペアでは確定時の位置にかかわらずキャレットを括弧内へ置く。IME変換中の不介入、TextKit 2、UTF-16 range、Undo / Redoの既存契約は維持する。
