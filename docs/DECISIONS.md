...
- **内容**:
  1. `POSTGRES_USER`（公式初期OID 10）のidentifierとsecretはコンテナ初期化専用とし、base Compose、runtime／通常migratorのDSN、environment、secret mount、runnerへ渡さない。公式identifier／secretと`bootstrap_admin` serviceはfresh専用`docker-compose.provision.yml`へ物理分離する。明示`provision` profileのone-shotは同一transaction／advisory lock内で公式OID 10 authority、固定DB名、v2 role不存在、FirstNormalObjectId以上のuser role／OID付きcatalog object不存在、OIDを持たないmutable catalogのfresh baselineを検証してからだけ一時`bootstrap_admin`を作成し、exact／legacy／unknown DBではcatalog変更0でfail closedする。
