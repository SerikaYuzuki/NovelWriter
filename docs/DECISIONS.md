...
- **実装状況 (2026-08-09)**: 別app target／scheme／bundle ID／既定保存root、通常版とのbuild graph分離、App-level local context、fake provider、provider-neutralな共有AI UI、Codex sidecar v1、manifest v1、exact SDKの合成CLI capture、Darwin native process supervisor、B3 packager／native verifier、D-050のB4-A compile-time approval契約まで実装した。production catalogは空で、実Codex／OpenRouter adapter、実CLI／network、approved runtime、exact Node inspector、suspended launch、interactive transport、immutable verify-to-use binding、Keychain、OS-level隔離、parent death後のprocess回収は未実装であり、本決定のExperimental Gateを完了した意味ではない。
...
- **実装状況 (2026-08-09)**: protocol mock、canonical manifest v1、`@openai/codex-sdk` 0.147.0の合成CLI capture、D-048 supervisor、D-049 packager／native verifier、D-050のB4-A compile-time approval契約を実装した。production catalogは意図的に空であり、exact Node inspector、suspended launch、interactive `hello`／`ready`／`start`、完全なloaded inventory、immutable verify-to-use binding、監査済みlauncher、実SDK／CLI、credential、network、OS-level隔離は未実装である。`codex_sdk` runtime modeは引き続き禁止する。
