...
- **AIの現在地**: `NovelAI`、EditorKit selection transaction、App-level local context、fake provider、provider-neutralな共有UI、`FUMINIWAExperimental` target分離、Codex sidecar v1、canonical deployment manifest v1、exact SDK 0.147.0の合成CLI capture、Darwin native process supervisor、arm64固定21-file packager、Experimental native manifest verifier、B4-A compile-time approval契約まで実装済み。production catalogは意図的に空で、承認済みcandidate／Node／SDK／CLIと実行経路は0件。実provider／CLI／network、exact Node inspector、suspended launch、interactive transport、complete loaded inventory、immutable verify-to-use、Keychain、OS-level隔離、parent death後の回収は未実装である(D-043 / D-046 / D-047 / D-048 / D-049 / D-050)
...
- **6-3（Codex SDK route、B4-A approval contract完了）**: fixed sidecar protocol v1、canonical manifest v1、exact SDK 0.147.0の合成CLI capture、Darwin process supervisor、固定21-file packager、Swift Experimental native verifierに加え、B4-Aのclosed compile-time approval契約とinventory roleを実装した。`CodexRuntimeApprovalPolicy`はcanonical／validated契約でapproval authorityではなく、`CodexRuntimeApprovalProposal`も非authority、`CodexApprovedRuntimeIdentity`はprivate initializer、唯一のauthorityであるnested production catalogは意図的に空である。deployment candidateと`evaluatedSource`／`resolutionMetadata`／`executable`／`conditional`／`provenance`／`requestData`／`operatingSystemTrust`／`forbidden`、content identityの`exactFile`／`boundedRequestData`／`operatingSystemProvided`／`forbidden`を区別する。次はB4-B exact Node inspector、B4-C suspended launch、B4-D interactive `hello`／`ready`／`start`、B4-E closed linker／broker／helperとOS isolationであり、B4-E後も残るcredential／lifecycle／配布Gateを通すまで`codex_sdk`はNO-GOである(D-047 / D-048 / D-049 / D-050)
...
一般公開を延期したため、直近の個人用AI実装は **Phase 6-3のCodex SDK sidecar隔離feasibility B4-B** である。B4-Aまでにpure domain、Editor transaction、共有orchestrator／UI、Experimental分離、sidecar protocol、canonical manifest v1、exact SDK 0.147.0の合成capture、Darwin supervisor、arm64固定21-file packager、Experimental native verifier、empty compile-time approval contractを完了した。production catalogは空で、candidate／self manifest／proposal／observed runtime／local probeを自動承認せず、anti-rollbackを実装済みとは扱わない。次はB4-B exact Node inspector、B4-C suspended actual-process launch、B4-D interactive `hello` → `ready` → `start`、B4-E closed linker／broker／helperとOS-level read／exec隔離の順に、complete loaded artifact inventoryとimmutable verify-to-useを固定する。その後も監査済みlauncher／parent-death、専用cwd／`CODEX_HOME`、environment allowlist、memory／CPU／process limit、Keychainを実証し、全Gate後にCodex adapter、続いて独立したOpenRouter adapterを同じUIへ接続する(D-046 / D-047 / D-048 / D-049 / D-050)。
