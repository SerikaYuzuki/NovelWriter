...
> **Chapter / Episodeモデル(D-028、UI-FIX-2a実装済み)**: `Chapter` は章タイトルと `[Episode]` を持つ構造、`Episode` はタイトル・本文・メモを持つ編集単位である。AppStateとUIはUI-FIX-2b/2cでEpisodeID選択へ移行するまで、章単位の互換accessorを使用する。
...
Phase 0 / 1 / 2 / 3 / 4 / 旧 Phase UI / Phase UI2 / Phase 4.5 / Toolbar-1 / Toolbar-2 / UI-FIX-1 / UI-FIX-2a は完了済み(→ 変更履歴)。次は **UI-FIX-2b「AppStateの話選択と操作」**。詳細な作業指示書は [UIFIX.md](UIFIX.md)。
