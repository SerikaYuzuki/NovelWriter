...
> **次期モデル(D-028、実装計画 [UIFIX.md](UIFIX.md))**: 現在の `Chapter` が本文とメモを直接持つ構造は、UI Fix で `Chapter { episodes: [Episode] }` へ移行する。`Episode` がタイトル・本文・メモを持つ編集単位になる。移行完了までは下記の現行モデルが実装と一致する。
...
Phase 0 / 1 / 2 / 3 / 4 / 旧 Phase UI / Phase UI2 / Phase 4.5 / Toolbar-1 / Toolbar-2 は完了済み(→ 変更履歴)。次は **UI-FIX-1「エディタ密度 + 8pt フォント」**。その後、D-028 の Chapter / Episode 階層へ順番に移行する。詳細な作業指示書は [UIFIX.md](UIFIX.md)。
