...
上部 chrome の次期設計は D-024 と [TOOLBAR.md](TOOLBAR.md) を正とする。Project Sidebar 上は標準の開閉、Outline 上は作品名 + 章数、Editor 上は一段の編集操作 + 右端の章内検索とする。現在のペイン内 `EditorTopBarView` と展開式検索行は実装移行後に撤去し、編集操作は macOS 標準の toolbar カスタマイズ対象にする。保存状態は下部 status bar、選択章名は Outline を正とし、上部で重複表示しない。
