...
Phase 0 / 1 / 2 は完了済み(→ 変更履歴)。次は Phase 3(基本操作強化)。優先度順:

1. 章タイトル編集UIを追加する
2. 章削除を実装する(確認ダイアログ付き)
3. Cmd+Q 時の即時保存(D-016 の既知の制限の解消。applicationShouldTerminate で dirty なら保存完了後に終了)
4. 章並べ替えの安定化(高速操作・保存タイミングの検証)
5. 検索ジャンプを実装する
6. スナップショット保存
7. 小さな負債: 各モジュールの `placeholderVersion` 定数の一括削除(scaffold の名残)
