...
1. **ContentView の分割** — 現在約1,400行まで肥大。インスペクタ各タブ(メモ/キャラクター/プロット/伏線/資料)と検索バーをファイル分割し、再利用可能な部品は NovelUI へ(9.4 の精神。機能追加とは独立にできる)
2. 章リスト・エディタ周りのパフォーマンス計測(長編・数十万字での体感確認)
