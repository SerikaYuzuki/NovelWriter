...
Phase 0 / 1 / 2 / 3 / 4 は完了済み(→ 変更履歴)。次は Phase 5(出力)。

Phase 4(小説執筆支援機能)の実行記録は [PHASE4.md](PHASE4.md) を参照。4-1〜4-6 すべて完了済み(Nice to have の未実施分は PHASE4.md のチェックボックスに残してある)。

Phase 5 着手前の負債返済候補:

1. 章リスト・エディタ周りのパフォーマンス計測(長編・数十万字での体感確認)

完了済みの負債返済:

- **ContentView の分割** — 約1,400行まで肥大していた `ContentView.swift` を画面骨格、検索バー、インスペクタ共通、キャラクター/プロット/伏線/資料タブ、共通表示部品へ分割
...
Phase 5 着手前の負債返済として `ContentView.swift` を分割。

- `ContentView.swift` を画面骨格と検索ジャンプ状態管理に集中させ、約1,400行から約300行へ縮小
- 検索バーを `SearchToolbarItems.swift` に分離
- インスペクタ共通コンテナを `InspectorView.swift` に分離
- キャラクター / プロット / 伏線 / 資料の各タブを専用ファイルに分離
- 章タイトル編集、文字数バー、操作メッセージを `ContentSupportViews.swift` に分離
