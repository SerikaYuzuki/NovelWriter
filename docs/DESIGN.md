...
完了済み(詳細は [UIDESIGN.md](UIDESIGN.md))。

- 単一ウィンドウ・3モード制(執筆 / キャラクター / プロット)
- キャラクターシート
...
Phase 0 / 1 / 2 / 3 / 4 / UI は完了済み(→ 変更履歴)。**次は Phase 5(出力)。**

Phase UI の実行記録は **[UIDESIGN.md](UIDESIGN.md)** を参照(単一ウィンドウ・3モード制、プロット=章レーンボード、キャラ=シート型 → D-019)。
