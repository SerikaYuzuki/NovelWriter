...
> **Chapter / Episodeモデル(D-028、UI-FIX-2a/2b実装済み)**: `Chapter` は章タイトルと `[Episode]` を持つ構造、`Episode` はタイトル・本文・メモを持つ編集単位である。AppStateは`selectedChapterID` / `selectedEpisodeID`を選択の正とし、UIはUI-FIX-2cで階層表示へ移行する。
...
新UIの画面構成は D-021 / D-024 と [UIDESIGN.md](UIDESIGN.md) / [TOOLBAR.md](TOOLBAR.md) が正である。左から Project Sidebar、Outline(content)、Editor(detail) を `NavigationSplitView` で並べ、下部に AI Assistant Panel を置く。Project Sidebar は 作品情報 / 企画 / 執筆 / プロット / 登場人物 / 世界観 / 資料 / 設定 を持つ固定気味のナビゲーションで、content 列は現在のセクションに応じた一覧を表示する。本文執筆では content に章一覧、detail に本文を出す。
...
Phase 0 / 1 / 2 / 3 / 4 / 旧 Phase UI / Phase UI2 / Phase 4.5 / Toolbar-1 / Toolbar-2 / UI-FIX-1 / UI-FIX-2a / UI-FIX-2b は完了済み(→ 変更履歴)。次は **UI-FIX-2c「階層型の執筆Outline + 追加menu」**。詳細な作業指示書は [UIFIX.md](UIFIX.md)。

Phase 5-1 は UI-FIX-5 完了後に開始する。現在の [PHASE5.md](PHASE5.md) は `Chapter` が本文を直接持つ前提なので、UI Fix 完了時に章見出し / 話見出しを持つ出力仕様へ更新する。上部 chrome の現行設計は [TOOLBAR.md](TOOLBAR.md)。
