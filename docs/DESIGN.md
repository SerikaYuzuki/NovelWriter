...
Phase 0 / 1 / 2 / 3 / 4 / 旧 Phase UI / Phase UI2 / Phase 4.5 / Toolbar-1 / Toolbar-2 / UI-FIX-1 / UI-FIX-2a / UI-FIX-2b / UI-FIX-2c / UI-FIX-3 / UI-FIX-4 / UI-FIX-5 は完了済み(→ 変更履歴)。UI-FIXの方向違いを修正するため、次は **UI-REV-1「Glass Outline」**。詳細な作業指示書は [UIREVISION.md](UIREVISION.md)。

Phase 5-1 の Chapter / Episode 出力仕様は [PHASE5.md](PHASE5.md) へ反映済みだが、実装開始はUI-REV-1〜9の完了後とする。作品→章→話の配列順、空章・空話、空タイトル、改行の共通規則は引き続き実装の正とする。上部chromeの修正はD-029と[UIREVISION.md](UIREVISION.md)を正とする。
