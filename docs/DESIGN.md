...
Phase 0 / 1 / 2 / 3 / 4 は完了済み(→ 変更履歴)。**次は Phase UI(GUI刷新)。Phase 5(出力)はその後。**

**Phase UI の作業指示書 = [UIDESIGN.md](UIDESIGN.md)**(単一ウィンドウ・3モード制、プロット=章レーンボード、キャラ=シート型 → D-019)。着手順は UI-1(基盤リファクタ。ContentView 分割の負債返済を含む)→ UI-2(キャラクターモード)→ UI-3(プロットモード)→ UI-4(執筆モード磨き込み)。**1サブフェーズ = 1PR、スタックPR禁止(前のPRのマージを待って main から切る)。**
