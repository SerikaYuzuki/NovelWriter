...
将来的に追加するもの: Project Sidebar 行 / Outline 行 / エディタTop Bar部品 / AI Assistant status item / 検索バー / 設定画面部品 / キャラクターカード / プロットカード
...
UIの正は `WorkspaceSelection`(Project Sidebar + Outline)に寄せる。旧3モード制で使った `AppMode` は、移行中の互換目的で残す場合も新UIの正にしない。
...
新UIの画面構成は D-021 と [UIDESIGN.md](UIDESIGN.md) が正である。左から Project Sidebar、Outline、Editor を並べ、下部に AI Assistant Panel を置く。Project Sidebar は 作品情報 / 企画 / 執筆 / プロット / 登場人物 / 世界観 / 資料 / 設定 を持つ固定気味のナビゲーションで、Outline は現在のセクションに応じた一覧を表示する。本文執筆では Outline に章・シーン一覧、Editor に本文を出す。

`ContentView` 自体は肥大化させず、状態の受け渡しと共通コマンドの入口に留める。本文エディタの実体は従来どおり EditorKit の `EditorView` であり、App側の `EditorPaneView` は章タイトル・検索・履歴・プレビュー・保存状態などのエディタ周辺UIを担当する。
...
Phase 0 / 1 / 2 / 3 / 4 / 旧 Phase UI / Phase UI2 は完了済み(→ 変更履歴)。**次は Phase 5(出力)。**
