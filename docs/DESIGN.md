...
public enum EditorAction {
    case allow
    case replace(String, caretOffset: Int)
}
...
プラグインは登録順に実行され、最初に `.replace` を返したプラグインで処理を確定する(順序が意味を持つことを明記しておく)。
...
`IndentRules.swift` は、改行時の字下げルールを定義する純ロジックである。

役割:

- 前行の先頭空白を取得する
- 開き括弧などの後に追加インデントが必要か判定する
- Editor本体から切り離してテスト可能にする
...
- キャラクター管理 / プロット管理 / フラグ管理 / 章メモ / 資料添付
...
1. Phase 0(git init、NovelKit パッケージ雛形、CI)
2. NovelCore モデル + `.novelpkg` Repository をテスト付きで実装
3. EditorPlugin基盤を正式実装する
4. IndentRules / IndentPlugin をテスト付きで実装する
5. EditorView を薄いFacadeに整理する
6. 章タイトル編集UIを追加する
7. 章削除・並べ替えを安定化する
8. 検索ジャンプを実装する
