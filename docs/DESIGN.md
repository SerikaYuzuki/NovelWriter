...
- **Apple Device Sync（D-063 source complete／local automated GO、Release NO-GO継続）**: private CloudKit + `CKSyncEngine`を`NovelSyncCloudKit` adapterとして使う。現行通常Appは、作品タイトル／あらすじ、章・話構造／順序／タイトル、本文／メモ、人物、プロット、伏線、世界観を`WorkSnapshot` v1として同期する。D-063はWorkID、bounded title projection＋full title digest／byte count、exact head ID＋snapshot digest／byte countを持つcatalogを追加し、remote-only workをapp-private packageへ初回materializeする。attachment／snapshot履歴／unknown root／端末設定は引き続き同期しない。別namespaceのWork wire v1、mutation receipt、whole revision `CKAsset`、作品全体3-way mergeと3面reviewを維持する。最終source freezeはnear-cap 270,439,704 bytesを含む`NovelSync` 142 / 142件（14 suites）、`NovelSyncCloudKit` 80 / 80件（20 suites）、freshな`./Scripts/check.sh`の`All checks passed`、署名済みDebug macOS／iOS buildのentitlement read-backを通過した。restored stateのない初回`CKSyncEngine`が同一accountを`.signIn`として通知しても永久`accountRequired`へ落とさずlive再検証後にreadyを維持する。D-063 live 4 record type＋保持中の旧Episode 3 typeのschema checklistとclean zone／type未生成bootstrapも固定した。署名済み実Mac AppではiCloud棚`available`／0作品まで確認したが、remote CRUD、paired native、Production schema deploy、実account switch、実OS kill、手動VoiceOver、Package Validator、External Change / Conflict、production migration／minimum-version fenceは未完了である。D-059〜D-061の既存回帰件数は履歴として維持し、mixed old／new client安全性を主張しない。SwiftDataはcanonical storeにせず、自前serverも置かない。macOSの非Sandboxは維持する(→ [DEVICE_SYNC.md](DEVICE_SYNC.md), D-059〜D-063)
...
- D-063最終source freezeはnear-cap 270,439,704 bytesを含む`NovelSync` 142 / 142件（14 suites）、`NovelSyncCloudKit` 80 / 80件（20 suites）、freshな`./Scripts/check.sh`の`All checks passed`、署名済みDebug macOS／iOS buildのentitlement read-backを通過。署名済み実Mac AppのiCloud棚は`available`／0作品。実remote CRUD／paired device／Production deployの証跡ではない
...
- 最終source freezeは`NovelSync` 142 / 142件（14 suites、near-cap 270,439,704 bytes）、`NovelSyncCloudKit` 80 / 80件（20 suites）、freshな`./Scripts/check.sh`、署名済みDebug macOS／iOS entitlement read-backを通過。初回same-account `.signIn`再検証、clean zone／type未生成bootstrapを固定し、署名済み実Mac AppのiCloud棚`available`／0作品を確認。Package Validator、External Change / Conflict、実CloudKit remote CRUD／paired device、Production deploy、実account switch／実OS kill／手動VoiceOver、production migration／minimum-version fenceはRelease NO-GOとして維持
