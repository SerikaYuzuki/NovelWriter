...
- CodexはSwift-native SDKでないため、provider実装／更新PRごとにreviewしたstable non-alpha TypeScript SDKをsemver rangeなしでexact pinしたNode sidecarを候補とする。2026-08-09の調査baselineは`0.147.0`である。B3ではarm64固定21-file packagerとExperimental native manifest verifierをidentity candidateとして実装し、B4-Aではcandidate生成から独立したempty production catalog付きapproval契約を固定した。B4-Bではcanonical raw path／`realpath`／`F_GETPATH`、owner／mode／`nlink`／size／SHA-256、strict thin／fat Mach-O、no-network Security validity／requested architecture別CDHashを512 MiB上限で観測する非実行Experimental native inspectorを追加した。candidate／self manifest／B4-B observation／local probeを自動承認せず、invalid signatureやuniversal Mach-Oの観測も実行許可ではなく、anti-rollbackも未実装である。B4-C suspended actual-process identity、B4-D interactive content Gate、B4-E closed linker／broker／helperとOS-level read／exec隔離でcomplete inventoryとimmutable verify-to-useを順に固定する。個人用Experimentalでもrequest専用empty cwd + `skipGitRepoCheck: true`／専用`CODEX_HOME`、environment allowlist、Keychain、OS-level file隔離、cancel後のprocess tree回収は緩めない。arm64／x86_64、bundling、nested signing／notarizationは公開Release Gateへ延期する。実repositoryや検査回避用の偽Git repositoryをcwdにしない
...
- **AIの現在地**: `NovelAI`、EditorKit selection transaction、App-level local context、fake provider、provider-neutralな共有UI、`FUMINIWAExperimental` target分離、Codex sidecar v1、canonical deployment manifest v1、exact SDK 0.147.0の合成CLI capture、Darwin native process supervisor、arm64固定21-file packager、Experimental native manifest verifier、B4-A compile-time approval契約、B4-B非実行exact Node inspectorまで実装済み。production catalogは意図的に空で、B4-BのSHA／Mach-O／Security観測は非authority値、承認済みcandidate／Node／SDK／CLIと実行経路は0件。実provider／CLI／network、Node version、suspended actual-process identity、interactive transport、complete loaded inventory、immutable verify-to-use、Keychain、OS-level隔離、parent death後の回収は未実装である(D-043 / D-046 / D-047 / D-048 / D-049 / D-050 / D-051)
...
- **6-3（Codex SDK route、B4-B exact Node inspector完了）**: fixed sidecar protocol v1、canonical manifest v1、exact SDK 0.147.0の合成CLI capture、Darwin process supervisor、固定21-file packager、Swift Experimental native verifierに加え、B4-Aのclosed compile-time approval契約とinventory role、B4-Bの非実行exact Node native inspectorを実装した。`CodexRuntimeApprovalPolicy`はcanonical／validated契約でapproval authorityではなく、`CodexRuntimeApprovalProposal`も非authority、`CodexApprovedRuntimeIdentity`はprivate initializer、唯一のauthorityであるnested production catalogは意図的に空である。inspectorはcanonical raw path／`realpath`／`F_GETPATH`、owner／mode／`nlink`／size／SHA-256、strict thin／fat Mach-O、no-network Security validity／requested architecture別CDHashを512 MiB上限で観測するが、universal Mach-Oを含む非authority値からpath／FD／launch capabilityを生成せず、production catalogは空のままである。次はB4-C suspended actual-process identity、B4-D interactive `hello`／`ready`／`start`、B4-E closed linker／broker／helperとOS isolationである。Node version、same-user post-verify swap、complete inventory／immutable bindingは未達であり、B4-E後も残るcredential／lifecycle／配布Gateを通すまで`codex_sdk`はNO-GOである(D-047 / D-048 / D-049 / D-050 / D-051)
...
一般公開を延期したため、直近の個人用AI実装は **Phase 6-3のCodex SDK sidecar隔離feasibility B4-C** である。B4-Bまでにpure domain、Editor transaction、共有orchestrator／UI、Experimental分離、sidecar protocol、canonical manifest v1、exact SDK 0.147.0の合成capture、Darwin supervisor、arm64固定21-file packager、Experimental native verifier、empty compile-time approval contract、非実行exact Node inspectorを完了した。production catalogは空で、candidate／self manifest／proposal／B4-B observation／local probeを自動承認せず、anti-rollbackを実装済みとは扱わない。次はB4-C suspended actual-process identity、B4-D interactive `hello` → `ready` → `start`、B4-E closed linker／broker／helperとOS-level read／exec隔離の順に、complete loaded artifact inventoryとimmutable verify-to-useを固定する。その後も監査済みlauncher／parent-death、専用cwd／`CODEX_HOME`、environment allowlist、memory／CPU／process limit、Keychainを実証し、全Gate後にCodex adapter、続いて独立したOpenRouter adapterを同じUIへ接続する(D-046 / D-047 / D-048 / D-049 / D-050 / D-051)。
