...
現在のAI実装で完成扱いにできる範囲は、`NovelAI`のpure domain、EditorKit／Appのlocal transactionとstale検査、fake provider、provider-neutralな共通UI、Codex sidecar v1、canonical deployment manifest v1、exact SDK 0.147.0の合成CLI capture、合成helper用Darwin native supervisor、B3のarm64固定21-file packagerとExperimental native verifier、B4-Aの`CodexRuntimeApprovalPolicy`／非authority `CodexRuntimeApprovalProposal`／private-init `CodexApprovedRuntimeIdentity`とempty nested `ProductionCatalog`、B4-Bの非実行`CodexNodeExecutableInspector`、B4-Cのprobe-only `CodexSuspendedProcessIdentityInspector`までである。packagerはcaller supplied allowlistを受けず、SDK／CLI／darwin-arm64 packageのexact metadata／lock SRIを実行せずに検査し、固定fileを新規rootへ実コピーしてsource bytesのhashとdestination manifestを一致させる。Swift verifierはNode oracle（digest `15b98ccf…d35288`、278 bytes）と一致し、canonical path、symlink／hardlink／special file／危険mode、resource cap、走査中mutationをfail-closedにする。expected digestはself fileではなく独立引数である。B4-Cは固定argv／empty environment／empty cwd／null stdio／新PGIDの`START_SUSPENDED` childについてactual PIDのprocess／dynamic code identityを照合し、成功時もresumeせずkill／direct reapしてarchitecture／CDHashだけを返す。B4-B inspectorはcanonical raw path／`realpath`／`F_GETPATH`、owner／mode／`nlink`／size／SHA-256、strict thin／fat Mach-O、no-network Security validity／requested architecture別CDHashを512 MiB上限で観測するが、universal Mach-Oを含むobservationは非authorityでpath／FD／launch capabilityを返さない。

B3の`candidateRootDigest`、root内のself manifest、packager自身が再計算したdigest、B4-Aのproposal、B4-B／B4-C observation、observed runtime／local probeはいずれもproduction approvalではない。production catalogは意図的に空で、承認済みcandidate／Node／SDK／CLIと実行経路は0件である。packagingがdestination作成後に失敗した場合はrecursive cleanupせず`partial_destination_retained`としてrootを残すため、呼出側がそのrootを実行不能として手動で隔離・削除する。既存destinationは変更も削除もしない。B4-Cはactual-process identityのprobeまで完了したが、結果をapproval／capabilityへ昇格させずproduction catalogは空のままである。Node version、完全なloaded artifact inventory、interactive `hello`／`ready`／`start`、検証済みbytesとimport／path-based spawnのimmutable binding、same-userによるinspection return後のswap解決はB4-D〜E以降に残る。policy generationだけで旧app／旧catalogへのanti-rollbackを実装済みとは扱わない。実SDK／CLI、credential、network、OS sandbox、実原稿を使っていないため、これはCodex／OpenRouterが利用可能または公開可能という意味ではない。
