...
| AI technical contract | `NovelAI`、EditorKit transaction、App local context、送信前／適用前stale、one-shot、fake provider、共通UI、Codex sidecar v1、manifest v1、exact SDK 0.147.0の合成CLI capture、Darwin native supervisor、arm64固定21-file packager、Experimental native manifest verifier、B4-A compile-time approval契約、B4-B非実行exact Node inspectorを実装。通常版とのbuild graphを機械監査 | production catalogは意図的に空。B3 candidate／self manifest／B4-B observation／local probeは非authority。実provider／CLI／network、approved runtime、Node version、suspended actual-process identity、interactive transport、immutable verify-to-use、Keychain、OS-level隔離、parent death後の回収は未実装。`.novelpkg`は変更しない |
...
現在のAI実装で完成扱いにできる範囲は、`NovelAI`のpure domain、EditorKit／Appのlocal transactionとstale検査、fake provider、provider-neutralな共通UI、Codex sidecar v1、canonical deployment manifest v1、exact SDK 0.147.0の合成CLI capture、合成helper用Darwin native supervisor、B3のarm64固定21-file packagerとExperimental native verifier、B4-Aの`CodexRuntimeApprovalPolicy`／非authority `CodexRuntimeApprovalProposal`／private-init `CodexApprovedRuntimeIdentity`とempty nested `ProductionCatalog`、B4-Bの非実行`CodexNodeExecutableInspector`までである。packagerはcaller supplied allowlistを受けず、SDK／CLI／darwin-arm64 packageのexact metadata／lock SRIを実行せずに検査し、固定fileを新規rootへ実コピーしてsource bytesのhashとdestination manifestを一致させる。Swift verifierはNode oracle（digest `15b98ccf…d35288`、278 bytes）と一致し、canonical path、symlink／hardlink／special file／危険mode、resource cap、走査中mutationをfail-closedにする。expected digestはself fileではなく独立引数である。B4-B inspectorはcanonical raw path／`realpath`／`F_GETPATH`、owner／mode／`nlink`／size／SHA-256、strict thin／fat Mach-O、no-network Security validity／requested architecture別CDHashを512 MiB上限で観測するが、universal Mach-Oを含むobservationは非authorityでpath／FD／launch capabilityを返さない。

B3の`candidateRootDigest`、root内のself manifest、packager自身が再計算したdigest、B4-Aのproposal、B4-B observation、observed runtime／local probeはいずれもproduction approvalではない。production catalogは意図的に空で、承認済みcandidate／Node／SDK／CLIと実行経路は0件である。packagingがdestination作成後に失敗した場合はrecursive cleanupせず`partial_destination_retained`としてrootを残すため、呼出側がそのrootを実行不能として手動で隔離・削除する。既存destinationは変更も削除もしない。B4-Bはpath上のexact Node executableを非実行で観測するところまで完了した。Node version、完全なloaded artifact inventory、suspended actual-process検査、interactive `hello`／`ready`／`start`、検証済みbytesとimport／path-based spawnのimmutable binding、same-userによるinspection return後のswap解決はB4-C〜E以降に残る。policy generationだけで旧app／旧catalogへのanti-rollbackを実装済みとは扱わない。実SDK／CLI、credential、network、OS sandbox、実原稿を使っていないため、これはCodex／OpenRouterが利用可能または公開可能という意味ではない。
