...
| AI technical contract | `NovelAI`、EditorKit transaction、App local context、送信前／適用前stale、one-shot、fake provider、共通UI、Codex sidecar v1、manifest v1、exact SDK 0.147.0の合成CLI capture、Darwin native supervisor、arm64固定21-file packager、Experimental native manifest verifier、B4-A compile-time approval契約、B4-B非実行exact Node inspector、B4-C probe-only suspended actual-process identity、B4-D abstract mock interactive sequencingを実装。通常版とのbuild graphを機械監査 | production catalogは意図的に空。B3 candidate／self manifest／B4-B／B4-C observation／local probeは非authority。B4-Cはresumeせずkill／direct reapし、B4-Dへ変換しない。B4-Dの具象production channel／factory／callsite、実provider／CLI／network、approved runtime、Node version、immutable verify-to-use、Keychain、OS-level隔離、parent death後の回収は未実装。`.novelpkg`は変更しない |
...
現在のAI実装で完成扱いにできる範囲は、`NovelAI`のpure domain、EditorKit／Appのlocal transactionとstale検査、fake provider、provider-neutralな共通UI、Codex sidecar v1、canonical deployment manifest v1、exact SDK 0.147.0の合成CLI capture、合成helper用Darwin native supervisor、B3のarm64固定21-file packagerとExperimental native verifier、B4-Aの`CodexRuntimeApprovalPolicy`／非authority `CodexRuntimeApprovalProposal`／private-init `CodexApprovedRuntimeIdentity`とempty nested `ProductionCatalog`、B4-Bの非実行`CodexNodeExecutableInspector`、B4-Cのprobe-only `CodexSuspendedProcessIdentityInspector`、B4-DのExperimental-only／mock-only `CodexSyntheticInteractiveTransport`までである。packagerはcaller supplied allowlistを受けず、SDK／CLI／darwin-arm64 packageのexact metadata／lock SRIを実行せずに検査し、固定fileを新規rootへ実コピーしてsource bytesのhashとdestination manifestを一致させる。Swift verifierはNode oracle（digest `15b98ccf…d35288`、278 bytes）と一致し、canonical path、symlink／hardlink／special file／危険mode、resource cap、走査中mutationをfail-closedにする。expected digestはself fileではなく独立引数である。B4-Cは固定argv／empty environment／empty cwd／null stdio／新PGIDの`START_SUSPENDED` childについてactual PIDのprocess／dynamic code identityを照合し、成功時もresumeせずkill／direct reapしてarchitecture／CDHashだけを返す。B4-B inspectorはcanonical raw path／`realpath`／`F_GETPATH`、owner／mode／`nlink`／size／SHA-256、strict thin／fat Mach-O、no-network Security validity／requested architecture別CDHashを512 MiB上限で観測するが、universal Mach-Oを含むobservationは非authorityでpath／FD／launch capabilityを返さない。B4-Dはcontent-free open、isolated exact `ready`後だけのsealed `start`、terminal／EOF、cancel／cleanupをin-memory channelで固定するが、具象production channel／factory／callsiteを持たず、B4-C childをresumeまたはtransportへ変換しない。

B3の`candidateRootDigest`、root内のself manifest、packager自身が再計算したdigest、B4-Aのproposal、B4-B／B4-C observation、observed runtime／local probeはいずれもproduction approvalではない。production catalogは意図的に空で、承認済みcandidate／Node／SDK／CLIと実行経路は0件である。packagingがdestination作成後に失敗した場合はrecursive cleanupせず`partial_destination_retained`としてrootを残すため、呼出側がそのrootを実行不能として手動で隔離・削除する。既存destinationは変更も削除もしない。B4-Cはactual-process identityのprobeまで完了したが、結果をapproval／capabilityへ昇格させずproduction catalogは空のままである。B4-Dはinteractive sequencingまで完了したがabstract mock channelだけであり、actual process identity／approval／loader closure／OS containmentと結合していない。Node version、完全なloaded artifact inventory、検証済みbytesとimport／path-based spawnのimmutable binding、same-userによるinspection return後のswap解決はB4-E以降に残る。policy generationだけで旧app／旧catalogへのanti-rollbackを実装済みとは扱わない。実SDK／CLI、credential、network、OS sandbox、実原稿を使っていないため、これはCodex／OpenRouterが利用可能または公開可能という意味ではない。
...
Codex adapterはprovider実装／更新時に再確認したstable（非alpha）TypeScript SDKのexact versionを使う署名済みbundled Node sidecarを候補とする。2026-08-09の調査baselineは`0.147.0`で、合成captureでは入力stdinとexact schemaを保持できる一方、literal U+2028／U+2029を含む同じCLI出力がNode 22.23.1ではexact round-tripしNode 26.4.0ではSDK内部で分断される差、生stderr／errorがSDK例外へ入り、Abortがdirect childへのsignalに留まることを確認した。これらのexact local probe identityは互換性証拠であってapprovalではなく、Node 26.4.0の現組合せはNO-GOである。B2 native supervisorは合成helperに対するprocess-group signal、direct child reap、group空の瞬間観測まで、B3 packager／verifierはarm64 deployment identity candidateまで、B4-Aはempty compile-time approval contractまで、B4-Bは512 MiB上限の非実行exact Node path／SHA／Mach-O／Security observationまで、B4-Cはresumeしないsuspended actual-process identity probeまで、B4-Dはabstract mock interactive sequencingまでを固定した。B4-C timeoutはbest-effort lifecycle境界でasync hard return上限ではなく、same-uid外部`SIGCONT`、mapped-vnode／in-place mutation、B4-B SHA／approvalとの結合は未達である。次はB4-E closed execution closure／native broker／helperとOS-level read／exec隔離で完全なloaded inventoryとimmutable bindingを固定する。その後もrequest専用empty cwdと`skipGitRepoCheck: true`／専用`CODEX_HOME`、environment allowlist、Keychain、監査済みlauncher／parent-death境界、arm64／x86_64、nested signing、Hardened Runtime、公証を実機で証明するまで非出荷・UI非表示とする。実repositoryや検査回避用の偽Git repositoryをcwdにしない。公開SDKにephemeral thread optionが確認できないため、FUMINIWA側のresultがmemory onlyでも「履歴非保持」と表現しない。
