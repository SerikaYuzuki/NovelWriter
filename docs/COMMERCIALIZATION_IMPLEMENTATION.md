...
| AI technical contract | `NovelAI`、EditorKit transaction、App local context、送信前／適用前stale、one-shot、fake provider、exact preview／確認／cancel／diff／Copy／明示Applyの共通UI、Codex sidecar v1のNode／Swift mock、manifest v1のNode primitive、exact SDK 0.147.0の合成CLI captureを実装。別target／bundle／保存rootの`FUMINIWAExperimental`だけに含め、通常版とのbuild graphを機械監査 | 実Codex／OpenRouter provider、実CLI／network、native supervisor／manifest verifier、Keychain、OS-level隔離は未実装。`.novelpkg`は変更しない |
...
現在のAI実装で完成扱いにできる範囲は、`NovelAI`のpure domainに加え、EditorKitのIME／surface／revision／Undoを守るselection transaction、document session／episode／source digestをproviderへ渡さないApp local context、送信前／適用前stale、one-shot、cancel後の遅延完了破棄、fake provider、provider-neutralな共通UI、本文送信前attestationを持つCodex sidecar v1のNode／Swift mock、canonical deployment manifest v1のNode primitive、exact SDK 0.147.0を合成CLIだけで駆動するcaptureまでである。共通UIはexact preview、requestごとの確認、進行／cancel、原文との差分、stale、Copy、明示Applyを提供し、`FUMINIWAExperimental`だけに存在する。通常版は`NovelAI` dependency、compile flag、Experimental sourceを生成project監査で除外する。合成captureは実CLI、credential、network、実provider processを起動せず、packager／native verifier／supervisorも持たないため、これはCodex／OpenRouterが利用可能または公開可能という意味ではない。
...
Codex adapterはprovider実装／更新時に再確認したstable（非alpha）TypeScript SDKのexact versionを使う署名済みbundled Node sidecarを候補とする。2026-08-09の調査baselineは`0.147.0`で、合成captureでは入力stdinとexact schemaを保持できる一方、literal U+2028／U+2029を含む同じCLI出力がNode 22.23.1ではexact round-tripしNode 26.4.0ではSDK内部で分断される差、生stderr／errorがSDK例外へ入り、Abortがdirect childへのsignalに留まることを確認した。Node 26.4.0の現組合せはNO-GOであり、互換Nodeを選んでもnative process-group supervisor等のGateは残る。SDK／依存／Node／Codexの独立hash固定、request専用empty cwdと`skipGitRepoCheck: true`／専用`CODEX_HOME`、environment allowlist、Keychain、OS-level file-read隔離、cancel後のgroup kill／direct child reap／group空、arm64／x86_64、nested signing、Hardened Runtime、公証を実機で証明するまで非出荷・UI非表示とする。実repositoryや検査回避用の偽Git repositoryをcwdにしない。公開SDKにephemeral thread optionが確認できないため、FUMINIWA側のresultがmemory onlyでも「履歴非保持」と表現しない。
