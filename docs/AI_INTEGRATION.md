...
**状態: D-043で承認した非出荷契約 / provider・sidecar・UIは未実装**

本書は、ふみにわ（FUMINIWA）へAI支援を追加するときの実装境界と安全条件を定める。個別判断は[DECISIONS.md](DECISIONS.md)のD-040 / D-043、開発順は[DESIGN.md](DESIGN.md)と[COMMERCIALIZATION_IMPLEMENTATION.md](COMMERCIALIZATION_IMPLEMENTATION.md)を正とする。
...
10. **未確認の保持／利用上限をUIで補わない。** provider／service側の保持期間と学習利用、SDK／CLIがlocalに作るartifactの場所・範囲・保持期間、providerの料金単位とrequest上限は一次資料と実測で確認する。これは出荷UI前の技術Gateであり、未確認値を「履歴なし」「無料」「上限あり」等として表示しない。
...
- Adapterはconfirmed requestの`applicationPrompt`と`applicationResponseSchema`を文字列／byte変換以外で再構築、template化、追記せず、そのexact contentをproviderへ渡す。選択本文以外の原稿データをhidden promptやmetadataへ入れない。
...
- confirmed requestはdomain所有executorのone-shot leaseを共有し、copyや並行呼出しでも最初の1回だけproviderを開始する。再試行は同じconfirmed値を再利用せず、新しいpreviewと明示確認から始める。
...
providerが受け取れるのは、明示確認済みの`AIApplicationPayload`を封印したoutboundだけである。初回の`AIConfirmedRequest.outbound`が含むのは、previewしたprovider descriptor、purpose、application instruction ID、domainが生成した単一`applicationPrompt`、response schema ID、exact `applicationResponseSchema`、hard budget、promptとschemaを合計したapp-provided inputの文字数／UTF-8 byte数だけとする。raw instructionとexact selected textを別口でconfirmed outboundへ持たせず、previewの内訳から封印済みpromptを再結合しない。request IDを含むlocal routing情報が必要な場合もadapterまたは呼び出し側のmemoryで管理し、providerへ原稿identityとして送らない。adapterはconfirmed outboundのprompt／schemaにhidden context、metadata、instructionを追加しない。
...
最初の実装PRで許可するのは、providerとUIから独立した`NovelAI`のprovider-neutral outbound契約だけである。名称を含む実際のtarget追加はそのPRのdiffを正とし、本書だけで存在を主張しない。
...
- adapterの外側で不変O(1)のprovider descriptorを照合し、confirmationのone-shot lease、cancel済み／stream破棄後のprovider開始拒否、executor呼出しからのwall-clock timeoutを所有するdomain executor
...
SDK固有型やHTTP／processのerror型をdomain APIへ漏らさない。provider adapterはdomain protocolへ適合し、AppDependenciesが明示的に1つを選んで注入する。provider descriptorは実行中に変化せず、I/O、lock待機、actor hopを行わないO(1)の値とする。完了usageの`outputTokens`は必須かつ非負、`inputTokens`は不明なら省略可能だが存在時は非負とし、欠損／不正値はtyped failureにする。usageは生成後の事後報告であって、それ単独では費用capにならない。domain層自身はfallback、retry、provider選択、永続化を行わない。stale判定は次のEditor bridge PRでlocal-only validatorとして追加し、confirmed outboundの型をlocal identityで太らせない。

## 5. Provider順序と分離

実装順は次のとおりとする。これは自動fallback順ではない。

1. **Codex SDK adapter**: 第一候補。公式TypeScript SDKを、署名済みのbundled Node sidecarから使用する案を技術検証する。
...
## 6. Codex Node sidecarの非出荷Gate

公式のCodex TypeScript SDKはNode.js向けであり、Swift-native SDKではない。[公式README](https://github.com/openai/codex/tree/main/sdk/typescript)はSDKがCodex CLIをspawnし、JSONLで通信する構成とNode.js 18以降を示している。FUMINIWAではSwiftからSDKやCLIを直接呼ぶのではなく、固定protocolを持つ署名済みNode sidecarを候補とする。

採用候補は公式npmのstable（非alpha）`0.147.x`系列とする。ただしrange指定（`^0.147.0`等）は使わず、provider実装PRで公式公開物を再確認し、採用するexact versionを固定する。次のすべてを実機で証明するまで、sidecarとそれを使うUIは非出荷・非表示とする。

### 6.1 Supply chainと配布物

- npm installやruntime downloadを利用者環境で行わない。lockfileとvendor済み成果物から再現可能に組み立てる。
- Codex SDK、全transitive dependency、Node runtime、Codex CLI executableのexact versionと公式integrity／cryptographic hashをallowlistで固定し、起動前にも改ざんを検出する。
- Node runtime、sidecar script/bundle、Codex CLI、必要なnative artifactをアプリbundle内の決めた場所にのみ同梱する。
- helperとすべてのnested executable／libraryをDeveloper IDで適切に署名し、Hardened Runtime、Archive、notarization、stapling、Gatekeeper検証をアプリ全体で通す。
- arm64とx86_64の各clean Macで、初回起動、実request、cancel、更新後起動を検証する。片方のarchitectureだけの成功でuniversal配布可能としない。
...
- tool、shell、web search、skill、MCP、additional directoryを無効化し、校正用の固定input/output schema以外を受け付けない。
...
- domainが強制するapp-provided input／raw response／delta／decoded resultの文字・UTF-8 byte上限とwall-clock timeoutに加え、adapterはSDKの`maximumOutputTokens`相当parameterをconfirmed budgetから必ず設定する。実送信直前のSDK requestをcaptureするテストで値を確認し、usageの事後検査だけへ依存しない。
...
- confirmed budgetの`maximumOutputTokens`を上流parameterへ設定し、streaming、cancellation、usage reportingをすべて満たす。完了はraw structured outputとusageをdomainのstrict decode境界へ渡す。
...
- 利用者が確認したmodelとprovider routingを固定し、OpenRouterの自動model／provider fallbackを無効にする。指定先が使えなければerrorで終了する。
- data collection拒否やZDR等のrequest-level制約を採用する場合は、実際に送るparameterとrouting結果をテストし、確認できた状態だけをpreviewへ表示する。
...
requestを閉じる、作品sessionが変わる、またはアプリが終了すると、FUMINIWAが保持するpreview／result／diffを破棄する。SDK／provider側の保持はこの表の対象外である。provider／service側の保持期間と学習利用、local SDK／CLI artifactの場所・範囲・保持期間、providerの料金単位とrequest上限は出荷UI前に根拠を固定し、送信前に確認可能にする。現在はいずれも未実装Gateであり、価格戦略や契約判断へ拡張せず技術表示の完成条件として扱う。
...
2. **Editor bridge**: IME確定済み選択snapshotをlocal operation contextとして保持し、session／surface／episode／range／source digestのstale判定と1 Undoの局所適用をfake providerで統合テストする。local identityをconfirmed outboundへ混ぜず、まだ出荷UIへ露出しない。
3. **Codex sidecar feasibility**: 固定SDK／Node／Codex、request専用empty cwd + `skipGitRepoCheck: true`／`CODEX_HOME`、environment allowlist、OS-level file isolation、cancel／kill／orphan、local artifactの場所・範囲・期間、arm64／x86_64を非出荷test harnessで実証する。
4. **Codex adapter**: sidecar fixed protocol、Keychain、typed error、upstream token／wire event／process resource limit、実送信直前capture、redacted diagnosticsをdomainへ接続する。署名・公証Gate未達なら出荷targetとUIへ含めない。
5. **選択範囲校正UI**: D-040のPackage Validator、External Change / Conflict、配布技術Gate後にexact preview、明示確認、cancel、diff、stale表示、明示適用を追加する。
6. **OpenRouter adapter**: Codexとは独立したPRで追加し、自動fallbackなしをfailure testで固定する。

D-040の開発順を変更しない。Package Validator GateとExternal Change / Conflict Gateは引き続き直近の出荷優先事項である。AIの純粋domainや隔離PoCは非出荷・UI非表示のまま並行してよいが、公開Gateを迂回したAI対応の宣言や入口追加はしない。
...
- 同じconfirmationのcopy／並行実行ではprovider startがexactly onceであり、2件目以降は送信前にtyped failureとなる。executor呼出し前／descriptor取得中のcancel、timeout、stream破棄ではproviderを開始しない
- 具体adapterは実送信直前のSDK／HTTP requestをcaptureし、sealed prompt／schemaを再構築／追記せず、hidden instruction／metadataを追加せず、confirmed `maximumOutputTokens`を上流parameterへ設定したことを検証できる
...
- domainのapp-provided input／raw response／delta／decoded resultの文字・UTF-8 byte上限、wall timeout、usage検査と、adapterのupstream token／wire event／process limitを別々のfailure testで再現できる
