...
extension IOSDocumentStore {
    ...
    private func revalidatedDeviceSyncClient(
        _ cached: IOSDeviceSyncClient,
        identity: IOSDeviceSyncEpisodeIdentity,
        expectedLookup: IOSDeviceSyncLookupIdentity,
        runtime: IOSDeviceSyncRuntime
    ) async throws -> IOSDeviceSyncClient? {
    ...
    private func observeRemoteDuringRefreshIfNeeded(
        ...
        return try await client.coordinator.observeRemoteBase(
            localContent: document.episode(identity.editingToken.episodeID)?.episode.content ?? "",
            createdAt: runtime.now()
        )
    }

    private func applyDeviceSyncRefreshFailure(
        _ client: IOSDeviceSyncClient,
        identity: IOSDeviceSyncEpisodeIdentity,
        offline: Bool
    ) async {
        guard deviceSyncContextIsCurrent(identity) else { return }
