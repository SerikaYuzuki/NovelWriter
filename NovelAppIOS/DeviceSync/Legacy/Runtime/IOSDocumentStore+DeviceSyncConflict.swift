...
extension IOSDocumentStore {
    func resolveDeviceSyncConflict(
        ...
        let submitted = IOSPendingDeviceSyncConflictResolution(
    ...
    private func continueDeviceSyncConflictResolution(
        _ submitted: IOSPendingDeviceSyncConflictResolution,
        identity: IOSDeviceSyncEpisodeIdentity,
    ...
    func installResolvedConflictContent(
        ...
        await documentOperationGate.perform { [weak self] in
            ...
            guard let committed = capturedCommittedContent(for: expectedIdentity),
                  ...
                  document.episode(expectedIdentity.editingToken.episodeID)?.episode.content == committed else {
                return false
            }
            ...
            let result = await saveCoordinator.performExclusiveAfterFlushing(flushAfter: true) {
                ...
                installDeviceSyncEpisodeContent(
                    content,
                    chapterID: expectedIdentity.editingToken.chapterID,
                    episodeID: expectedIdentity.editingToken.episodeID,
    ...
    func updateDeviceSyncIdentityAfterRemoteInstall(_ previous: IOSDeviceSyncEpisodeIdentity) {
