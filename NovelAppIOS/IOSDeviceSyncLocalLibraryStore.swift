...
enum IOSDeviceSyncLocalLibraryError: Error, Equatable, Sendable {
    case unsafeRoot
...
enum IOSDeviceSyncLocalLibraryState: String, Codable, Hashable, Sendable {
    case reservedForPublish
    case publishPending
    case synced
    case remoteOpenPending
    case needsReview
    /// Account scopeを再確認するまでremote authorityを持たない端末内copy。
    case accountQuarantined
    /// 旧iOS app-private packageを新WorkIDへ明示取り込み済み。bytesは保全する。
    case legacyPreserved
}

struct IOSDeviceSyncLocalPackageAttestation: Codable, Hashable, Sendable {
    let documentID: UUID
    ...
    init(document: NovelDocument, updatedAt: Date) throws {
        ...
        // Digest input is already-validated canonical UTF-8.
...
struct IOSDeviceSyncLocalLibraryRecord: Codable, Hashable, Sendable, Identifiable {
    var id: SyncWorkID {
        workID
    }
    ...
    var state: IOSDeviceSyncLocalLibraryState
    var package: IOSDeviceSyncLocalPackageAttestation?
...
struct IOSDeviceSyncLocalLibraryInventory: Sendable {
    let records: [IOSDeviceSyncLocalLibraryRecord]
