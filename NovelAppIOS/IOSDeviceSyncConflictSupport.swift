...
enum IOSDeviceSyncMergeRecoveryPurpose: String, Codable, Hashable, Sendable {
    case reviewDraft
...
struct IOSDeviceSyncMergeRecoveryRecord: Codable, Hashable, Sendable {
    ...
    /// Missing purpose belongs to the legacy review-only format.
    let purpose: IOSDeviceSyncMergeRecoveryPurpose?
    ...
    init(
        ...
        content: String,
        purpose: IOSDeviceSyncMergeRecoveryPurpose = .reviewDraft
    ) {
...
struct IOSDeviceSyncConflictDraft: Equatable {
    ...
    var message: String {
        ...
        case .manualIntegrationRequired:
            "変更箇所が重なっているため、このiPhoneの本文を下書きにしています。もう一方の端末と見比べて統合してください。"
        case .recoveredIntegration:
