...
impl AuthPostgresRepository {
    ...
    fn hmac(&self, purpose: &str, token: &str) -> Result<Vec<u8>, AuthError> {
        let mut mac = HmacSha256::new_from_slice(self.token_hmac_key.as_slice())
    ...
    async fn find_receipt(
        ...
        let row = sqlx::query("SELECT command_kind,request_digest,response_status,response_ciphertext,state FROM auth_v1.auth_operations WHERE operation_id=$1")
        ...
        let bytes = self
            .vault
            .open("auth_receipt_v1", operation_id.as_str(), &secret)
            .await?;
        Ok(Some(AuthReceipt {
...
impl AuthRepository for AuthPostgresRepository {
    ...
    async fn load_challenge_for_update(
        ...
        let row = sqlx::query("SELECT operation_id,provider_config_id,audience,client_platform,state_hash,nonce_hash,phase FROM auth_v1.auth_challenges WHERE challenge_id=$1").bind(uuid).fetch_optional(&self.pool).await.map_err(Self::map_db)?.ok_or(AuthError::NotFound)?;
        Ok(ChallengeClaim {
    ...
    async fn mark_provider_result_known(
        ...
        let id = Self::uuid(challenge_id.as_str().trim_start_matches("challenge_"))?;
        let result = sqlx::query("UPDATE auth_v1.auth_challenges SET phase='providerResultKnown',provider_result_ciphertext=$2 WHERE challenge_id=$1 AND phase='providerCallStarted'").bind(id).bind(sealed_result.ciphertext).execute(&self.pool).await.map_err(Self::map_db)?;
        if result.rows_affected() != 1 {
            return Err(AuthError::InvalidChallengePhase);
        }
        Ok(())
    ...
    async fn finish_exchange(
        ...
        let (account_id, tenant_id, epoch, fence, identity_id) = if let Some(row) = row {
            ...
            sqlx::query("INSERT INTO auth_v1.external_identity_secrets(identity_id,key_version,ciphertext) VALUES($1,$2,$3)").bind(iid).bind(subject_secret.key_version).bind(subject_secret.ciphertext).execute(&mut *tx).await.map_err(Self::map_db)?;
        ...
        let response = to_vec(&grant).map_err(|_| AuthError::Vault)?;
        let sealed = self
            .vault
            .seal("auth_receipt_v1", operation_id.as_str(), &response)
            .await?;
        sqlx::query("UPDATE auth_v1.auth_operations SET state='completed',response_status=200,response_ciphertext=$2,completed_at=now() WHERE operation_id=$1 AND command_kind='exchangeApple'").bind(op).bind(sealed.ciphertext).execute(&mut *tx).await.map_err(Self::map_db)?;
    ...
    async fn refresh(
        ...
        if let Some(row) = sqlx::query("SELECT response_ciphertext,response_status,request_digest FROM auth_v1.session_refresh_receipts WHERE operation_id=$1 FOR UPDATE").bind(op).fetch_optional(&mut *tx).await.map_err(Self::map_db)? {
            ...
            let encrypted: Vec<u8> = row.try_get("response_ciphertext").map_err(Self::map_db)?; let bytes=self.vault.open("auth_receipt_v1",request.operation_id.as_str(),&SealedSecret{key_version:1,ciphertext:encrypted}).await?; let grant: SessionGrant=serde_json::from_slice(&bytes).map_err(|_|AuthError::Vault)?; tx.commit().await.map_err(Self::map_db)?; return Ok(RefreshOutcome{grant:Some(grant),receipt:None,reused:false});
        }
        ...
        let sealed = self
            ...
            .await?;
        sqlx::query("INSERT INTO auth_v1.session_refresh_receipts(operation_id,family_id,presented_token_hmac,request_digest,response_ciphertext,response_status,expires_at) VALUES($1,$2,$3,$4,$5,200,now()+interval '90 days')").bind(op).bind(family).bind(self.hmac("refresh",&request.refresh_token)?).bind(request.request_digest.as_slice()).bind(sealed.ciphertext).execute(&mut *tx).await.map_err(Self::map_db)?;
        tx.commit().await.map_err(Self::map_db)?;
        Ok(RefreshOutcome {
            grant: Some(grant),
            receipt: None,
    ...
    async fn revoke_current_session(
        ...
        let sealed = self
            ...
            .seal("auth_receipt_v1", operation_id.as_str(), &response)
            .await?;
        sqlx::query("INSERT INTO auth_v1.auth_operations(operation_id,command_kind,request_digest,state,response_status,response_ciphertext,completed_at) VALUES($1,'revokeCurrentSession',$2,'completed',200,$3,now()) ON CONFLICT(operation_id) DO NOTHING").bind(Self::uuid(operation_id.as_str())?).bind(request_digest.as_slice()).bind(sealed.ciphertext).execute(&mut *tx).await.map_err(Self::map_db)?;
        tx.commit().await.map_err(Self::map_db)?;
        Ok(AuthReceipt {
            operation_id: operation_id.clone(),
